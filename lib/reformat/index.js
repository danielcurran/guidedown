...
const {
  ...
  formatBossCard, formatShopList, formatCharacterSheet,
...
function postProcess(md) {
  ...
  md = md.replace(/\n{3,}\*\*DUNGEON #\d/g, '\n\n**DUNGEON #');
  md = md.replace(/\n{3,}\*\*Boss:/g, '\n\n**Boss:');
  // Merge broken paragraphs: when a line ends with a lowercase letter
  // and the next line starts with a lowercase letter or digit, the source
  // author likely added a blank line for readability that the reformatter
  // preserved as a paragraph break. Merge them back.
  md = md.replace(/([a-z])\n\n([a-z0-9])/g, '$1 $2');
